...
void SYS_CLK_Initialize( const SYS_CLK_INIT const * clkInit )
{
    ...
    /* RODIV */
    PLIB_OSC_ReferenceOscDivisorValueSet ( OSC_ID_0, OSC_REFERENCE_4, 5 );
    /* ROTRIM */
    PLIB_OSC_ReferenceOscTrimSet ( OSC_ID_0, OSC_REFERENCE_4, 296 );
    ...
    // now the clock dividers are all set up, we can increase the processor speed to max
    // can't change SPLL while it's being used as clock, so switch to Posc temporarily
    SYS_DEVCON_SystemUnlock();
    OSCCONbits.NOSC = 2;            // Posc
    OSCCONSET = 1;
    SYS_DEVCON_SystemLock();
    SYS_DEVCON_SystemUnlock();
    PLIB_OSC_SysPLLMultiplierSelect( OSC_ID_0, 41 );
    SYS_DEVCON_SystemLock();
    SYS_DEVCON_SystemUnlock();
